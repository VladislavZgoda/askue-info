...
import { store } from '@/actions/App/Http/Controllers/UspdSimCardController';
...
import type { UspdSimCardsProps } from '@/types';

export default function Create({ uspd, simCards }: UspdSimCardsProps) {
    return (
        <div className="mx-auto max-w-xs p-2">
            <Form<{ sim_card_id: string }> action={store(uspd.id)} instant disableWhileProcessing>
                {({ errors, processing, resetAndClearErrors }) => (
                    <Card className="mx-auto mt-2 w-full max-w-xs">
                        <CardHeader>
                            <CardTitle>Привязать сим-карту к {uspd.model}</CardTitle>
                            <CardDescription>№{uspd.serial_number}</CardDescription>
                        </CardHeader>
                        <CardContent>
